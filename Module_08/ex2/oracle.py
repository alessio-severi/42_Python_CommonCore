...
def print_config() -> tuple[str | None, ...]:
    ...
    if (temp := config["Mode"][0]) != "Not configured":
        if not (temp == "development" or temp == "production"):
            config["Mode"][0] = "\033[33m" + "Invalid value" + "\033[0m"
            config["Log Level"][0] = ("\033[33mNot applicable\033[0m")
            config["Mode"][1] = None

        elif (temp1 := config["Log Level"][0]) != "Not configured":

            log_levels = ['CRITICAL', 'ERROR', 'WARNING', 'INFO', 'DEBUG']
            if ((temp == "development" and temp1 not in log_levels)
                    or (temp == "production" and temp1 not in log_levels[:4])):
                config["Log Level"][0] = ("\033[33mInvalid value\033[0m")
                config["Log Level"][1] = None

        if temp == "development":
            config["Mode"][0] = f"\033[34m{temp}\033[0m"
        elif temp == "production":
            config["Mode"][0] = f"\033[32m{temp}\033[0m"
...
def main() -> None:
    if no_import:
        return print(f"\n\033[31m{no_import}\033[0m\nLacking dotenv module."
                     " Install it with: 'pip -r requirements.txt'")
