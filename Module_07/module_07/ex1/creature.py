...
class Bloomelle(Creature, HealCapability):
    ...
    def heal(self, target: list[Creature]) -> str:
        if not (target and all(map(lambda x: target.count(x) == 1,
                                   target))):
            return "healing error"
...
class Shiftling(Creature, TransformCapability):
    ...
    def transform(self) -> str:
        if not self._base_form:
            return f"{self._name} is already in its sharper form!"
        self._base_form = False
        return f"{self._name} shifts into a sharper form!"

    def revert(self) -> str:
        if self._base_form:
            return f"{self._name} is already in its normal form!"
...
class Morphagon(Creature, TransformCapability):
    ...
    def transform(self) -> str:
        if not self._base_form:
            return f"{self._name} is already in its dragonic battle form!"
        self._base_form = False
        return f"{self._name} morphs into a dragonic battle form!"

    def revert(self) -> str:
        if self._base_form:
            return f"{self._name} is already in its stable form!"
        self._base_form = True
